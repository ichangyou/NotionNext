...
import { normalizeRecordMap } from '@/lib/notion/getPostBlocks'
import {
  evaluatePostContentQuality,
  isPublishedPostForList
} from './content-indexing'

// 由字符串派生一个稳定的正整数种子（FNV-1a），用于确定性地分散推荐位。
...
/**
 * 获取文章的关联推荐文章列表。
 * 策略：① 同标签优先（保留相关性）；② 不足 count 时用其余文章补足，覆盖长尾。
 * 补位从「以 post.id 派生的偏移量」开始环绕遍历，使不同文章页从不同位置取——
 * 把可索引文章页的内链均匀分散到长尾（缓解「已发现-尚未编入索引」的抓取优先级问题），
 * 同时对同一文章保持确定性、内链不抖动。
 * @param post
 * @param {*} allPosts
 * @param {*} count
...
export function getRecommendPost(post, allPosts, count = 6) {
  ...
  const candidates = (allPosts || []).filter(
    p => p && p.id !== post?.id && (p.type || '').indexOf('Post') >= 0
  )
  ...
  for (const p of candidates) {
    ...
    if (Array.isArray(p.tags) && p.tags.some(t => currentTags.includes(t))) {
      push(p)
    }
  }

  // 2) 用其余文章补足到 count；以 post.id 派生偏移量环绕遍历，分散内链到长尾
  if (result.length < count && candidates.length > 0) {
    const offset = seedFromString(post?.id || '') % candidates.length
    for (let k = 0; k < candidates.length && result.length < count; k++) {
      push(candidates[(offset + k) % candidates.length])
    }
  }
...
export async function processPostData(props, from) {
  ...
  if (props.post?.blockMap) {
    normalizeRecordMap(props.post.blockMap)
  }
  ...
  if (props.post?.blockMap?.block) {
    ...
    // 合并静态质量标记与正文长度校验结果
    props.post.contentQuality = evaluatePostContentQuality(props.post)
  ...
  const allPosts = props.allPages?.filter(
    page => isPublishedPostForList(page)
  )
