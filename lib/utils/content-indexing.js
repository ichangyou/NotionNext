...
export function isIndexableSlug(slug) {
  ...
  // Guard against JS serialization artifacts: slug segments must not be the literal string "undefined"
  if (normalizedSlug.split('/').some(seg => seg === 'undefined')) return false
...
export function getContentRedirectTargetForSlug(slug) {
  ...
  // 默认不重定向：排除清单只负责 noindex（+退出 sitemap/列表），页面对直接访问者仍可读。
  // 仅当显式配置了非首页的 CONTENT_REDIRECT_DEFAULT 时才兜底跳转；'/' 或空视为「不跳转」，
  // 避免把仅需 noindex 的薄文 301 到首页（造成 soft 404、读者无法访问）。
  const fallback = (BLOG.CONTENT_REDIRECT_DEFAULT || '').toString().trim()
  if (!fallback || fallback === '/') {
    return null
  }
  return normalizeRedirectDestination(fallback)
}
