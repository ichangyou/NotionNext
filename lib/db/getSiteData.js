...
import {
  sanitizeNavigationLink,
  sanitizeNavigationLinks
} from '@/lib/utils/navigation'
import {
  annotateContentQualityForPages,
...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  // 标记内容质量，用于 noindex/sitemap 过滤低质量与重复改写内容
  annotateContentQualityForPages(allPages)
  postCount = allPages.filter(isPublishedPostForList).length
...
function cleanPages(allPages, tagOptions) {
  ...
  allPages.forEach(page => {
    // 确保封面字段可序列化，避免 undefined 导致 SSG 序列化失败
    if (page && page.pageCoverThumbnail === undefined) {
      page.pageCoverThumbnail = null
    }
...
function getCustomNav({ allPages }) {
  ...
  if (allPages && allPages.length > 0) {
    allPages.forEach(p => {
      p.to = p.slug
      const navItem = sanitizeNavigationLink({
        icon: p.icon || null,
        name: p.title || p.name || '',
        href: p.href,
        target: p.target,
...
function getCustomMenu({ collectionData, NOTION_CONFIG }) {
  ...
  return sanitizeNavigationLinks(menus)
}
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  ...
  const rawCover = collection?.cover
    ? mapImgUrl(collection?.cover, collection, 'collection')
    : defaultPageCover
  // Unsplash random URLs are unreliable and produce different images each request
  const isRandomCover = rawCover && /unsplash\.com\/random/i.test(rawCover)
  const pageCover = isRandomCover ? defaultPageCover : rawCover
