...
export function adjustPageProperties(properties, NOTION_CONFIG) {
  ...
  if (properties.type === 'Post') {
    ...
    properties.href = (properties.slug ?? properties.id)?.trim?.() ?? properties.id
  } else if (properties.type === 'Menu' || properties.type === 'SubMenu') {
    // 菜单路径为空、作为可展开菜单使用
    properties.href = (properties.slug ?? '#')?.trim?.() ?? '#'
    properties.name = properties.title ?? ''
  }

  // 修复误导跳转：例如 article/https://foo.com 这类被错误拼接前缀的链接。
  if (
    typeof properties?.href === 'string' &&
    !isHttpLink(properties.href) &&
    /https?:\/\//i.test(properties.href)
  ) {
    properties.href = sliceUrlFromHttp(properties.href)
  }
  ...
  // 公众号关注门控属性（支持 checkbox、select 或 text 类型）
  // Notion 中可以添加一个名为 wechat_gate 的属性来控制单篇文章是否需要门控
  if (properties.wechat_gate !== undefined) {
    // 统一转换为布尔值
    const val = properties.wechat_gate
    if (typeof val === 'boolean') {
      properties.wechat_gate = val
    } else if (Array.isArray(val)) {
      // select/multi_select 类型，检查是否选中了"是"或类似值
      const selected = val[0]?.toLowerCase() || ''
      properties.wechat_gate = ['yes', 'true', '是', '开启', 'on'].includes(selected)
    } else if (typeof val === 'string') {
      const strVal = val.toLowerCase().trim()
      properties.wechat_gate = ['yes', 'true', '是', '开启', 'on', '1'].includes(strVal)
    } else {
      properties.wechat_gate = Boolean(val)
    }
  }
}
...
function generateCustomizeSlug(postProperties, NOTION_CONFIG) {
  // 去除slug首尾意外的空白字符，避免生成形如 "article/  foo" 的错误URL，
  // 导致干净路径(/foo 或 /article/foo)匹配不到文章而出现空白页
  if (typeof postProperties.slug === 'string') {
    postProperties.slug = postProperties.slug.trim()
  }
  // 外链不处理
