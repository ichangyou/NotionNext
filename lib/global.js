...
import { generateLocaleDict, initLocale, redirectUserLang,saveLangToLocalStorage } from './lang'
...
export function GlobalContextProvider(props) {
  ...
  // 实时跟随系统深浅色：仅当用户未手动切换过主题（localStorage 无记录）时生效
  useEffect(() => {
    ...
    const handleChange = e => {
      // 用户手动选择过就尊重手动，不再跟随系统
      if (loadDarkModeFromLocalStorage()) {
        return
      }
